Module PredictorUV
 ...
 !! for diffusive flux (it is decided by BetaVis). All extra   
 ...
 ! Date : 20.09.2019
    ...
    IMPLICIT NONE
    PRIVATE
    real(kind=dp),DIMENSION(:,:),pointer:: u,Uvolf
    real(kind=dp),DIMENSION(:,:),pointer:: v,Vvolf
    Real(kind=dp),DIMENSION(:,:),pointer:: p
    Real(kind=dp),PARAMETER,PRIVATE:: tol=1.d-14
    ...
    Subroutine Predictor_UV(PGrid,UGrid,VGrid,PCellO,UCellO,VCellO,PCell,      &
      ...
      !! The pressure cell at next time step (n+1)
      TYPE(Cell),INTENT(IN):: PCellO
      !! The pressure cell at current time step (n) 				
      TYPE(Cell),INTENT(IN),target:: UCell,VCell
      !! The velocities cell at next time step (n+1) 
      TYPE(Cell),INTENT(IN),target:: UCellO,VCellO
      !! The velocities cell at next time step (n)	
      TYPE(Variables),INTENT(IN),target:: TVar
      !! The state variables
      TYPE(Predictor),INTENT(INOUT):: Pred
      !! The predictor velocity 			
      TYPE(PoissonCoefficient),INTENT(INOUT):: PU,PV
      !! The cofficient for solving Poisson equation 	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable,INTENT(INOUT):: Flux_n1
      !! The total flux from the previous time step 			
      ...
      !! The indexes to indicate the position of particles in the velocity grid  	 
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: ConFluxEW,ConFluxNS
      !! The convective flux	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: DifFluxEW,DifFluxNS
      !! The diffusive flux	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: ExEDFluxEW,ExEDFluxNS,EDFluxEW,EDFluxNS
      !! The extra diffusive flux for cutcell  
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: CFluxEW,CFluxNS
      !! The density-based computation of the convective flux
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: MaFluxEW,MaFluxNS
      !! The mass flux for density-based computation 	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: VofFluxEW,VofFluxNS
      !! The volume fraction flux 
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable::MoParExCo
      !! The source term coefficient for coupling particles and fluid field  
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: FluxDiv
      !! The total divergent flux
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: GradP
      !! The pressure gradient 	 
      REAL(KIND=dp),DIMENSION(:,:),allocatable:: UFric,VFric
      !! The friction coefficient for velocities	  
      REAL(KIND=dp),DIMENSION(:,:),allocatable:: UWE,USN,VWE,VSN
      !! Boundary condition for implicit diffusive solver	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable:: matr
      !! Parameter for checking whether solver give an acceptable result
      REAL(KIND=dp),DIMENSION(:,:),allocatable:: rhm
      !! Parameter for checking whether solver give an acceptable result
      REAL(KIND=dp),DIMENSION(:,:),allocatable:: Uro,Vro
      !! The mass inside velocity cell 
      ...
      Uvolf=>UCellO%vof
      Vvolf=>VCellO%vof
      ! Compute the density for velocity cells	
      do i=1,Isize
        do j=1,Jsize
	...
	if((MoParExIJU(i,1)==-1.or.MoParExIJU(i,2)==-1).and.                   &
      ...
      ! Step 1: Calculate the convective flux
      if(itt==1) then
      ...
      ! Step 2: Calculate the diffusive flux
      ...
      do i = 1,Isize-1
        do j = 1,Jsize
          ...
          if(UCell%Posnu(i,j)/=-1) then
            ...
            ! Compute the density-based convective flux 
      ...
      ! Apply the mixing procedure 
      call Mixing_Procedure(UGrid,UCell,1,0,FluxDiv(:,:,1))
      ! Add diffusive flux to divergent flux 
      do i=1,Isize-1
        do j=1,Jsize
          if(UCell%Posnu(i,j)/=-1) then
      ...
      ! Add pressure gradient (can be disable for some cases )
      ...
      ! Add the particle momentum exchange to divergent flux for UCell
      do i=1,TraPar%np
        ii=MoParExIJU(i,1)
        jj=MoParExIJU(i,2)
      ...
      ! Solving for UCell (implicit diffusive flux)
      nullify(Uvolf,Vvolf)
      Uvolf=>UCell%vof
      Vvolf=>VCell%vof
    ...
    Subroutine SetBasicSolver(solver,precond) 
        !! The subroutine set up methods for HYPRE solver
        IMPLICIT NONE
	...
	!! The input number to determine this subroutine is applied for x or y direction  
          ...
          do j = jbeg,Jsize-iv
            if(TCell%Posnu(i,j)/=-1) then
              ...
              if(iu==1) then ! for UCell
                ! Coefficient for diffusive flux
		...
		! Coefficient for extra diffusive flux 
              ...
              ! Set up coefficient for boundary 
    ...
    subroutine SetVectors(b,x,par_b,par_x,PGrid,TGrid,TCell,CWE,CSN,IJFlux,    &
                                                            rhm,dt,iu,iv)
      !! The subroutine set up a right hand side vector for HYPRE solver
        INTEGER*8,INTENT(IN):: b,x,par_b,par_x
	!! The ID for vector
        INTEGER,INTENT(IN):: iu,iv
	!! The input number to determine this subroutine is applied for x or y direction  
	...
	!! The total flux which is set to be right hand side vector
        REAL(KIND=dp),DIMENSION(:,:),INTENT(INOUT):: rhm
	!! The output right hand side vector which is used for checking solver
    ...
    SUBROUTINE DeltaGetValues(x,TCell,Var,iu,iv)
        !! The subroutine get the result for HYPRE
        INTEGER*8,INTENT(IN):: x
	!! The ID for result vector
        INTEGER,INTENT(IN):: iu,iv
	!! The input number to determine this subroutine is applied for x or y direction  
        TYPE(Cell),INTENT(IN):: TCell
	!! The cell
	...
	!! The output variable 
        ...
        ilower = 0
        iupper = TCell%ExtCell
        local_size = TCell%ExtCell+1 ! Number of elements
    ...
    SUBROUTINE ModifiedConvectiveFlux(PGrid,UGrid,VGrid,PCell,UCell,VCell,     &
                                      flux,vb,idir,jdir)
      !! The subroutine computes the convective flux through velocity cell faces. 
      !! The method is first order upwind for whole computational domain except 
      !! the boundary cells, where, central scheme is applied.  		
      IMPLICIT NONE
      TYPE(Grid),INTENT(IN):: PGrid,UGrid,VGrid
      !! The grid	
      ...
      !! The parameter indicating the subroutine applied for x or y direction	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable,INTENT(INOUT):: flux
      !! The output convective flux 
      INTEGER(kind=it4b):: i,j
        ...
        do j = 1,Jsize+jdir
          if(idir==1) then
            uw = 0.5d0*(u(i,j)+u(i-1,j))
     	    ! For u being convective velocity and scalar velocity
            if(i==ibeg) then
    ...
    SUBROUTINE HighOrderConvectiveFluxForXDir(PGrid,UGrid,VGrid,UCell,         &
                                      VCell,vb,flux)
      !! The subroutine used the MUSCL scheme for computing the covective flux 
      !! in x direction. It is applied for both u and v.    
      !! The work is based on 'A MUSCL scheme on staggered grids with kinetic-like fluxes
    ...
    subroutine HighOrderConvectiveFluxForYDir(PGrid,UGrid,VGrid,UCell,         &
                                      VCell,vb,flux)
      !! The subroutine used the MUSCL scheme for computing the covective flux 
      !! in y direction. It is applied for both u and v.   	
      IMPLICIT NONE
      TYPE(Grid),INTENT(IN):: PGrid,UGrid,VGrid
      !! The grid	
      TYPE(Cell),INTENT(IN):: UCell,VCell
    ...
    subroutine FaceDensityFlux(PGrid,UGrid,VGrid,PCell,UCell,VCell,            &
      ...
      !! The subroutine compute the mass flux through cell face. 
      ...
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable,INTENT(INOUT):: flux
      !! The mass flux 	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable,INTENT(INOUT):: VFlux
      !! The mixture density at cell faces
      INTEGER(kind=it4b):: i,j
    ...
    subroutine DensityBasedConvectiveFlux(PGrid,UGrid,VGrid,PCell,UCell,VCell, &
      ...
      !! The subroutine computes convective fluxes through cell faces.
      IMPLICIT NONE
      TYPE(Grid),INTENT(IN):: PGrid,UGrid,VGrid
      !! The grid
      TYPE(Cell),INTENT(IN):: PCell,UCell,VCell
      ...
      INTEGER(kind=it4b),INTENT(IN):: idir,jdir
      !! The parameter indicating the subroutine applied for x or y direction 	 	
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable,INTENT(IN)::Vflux
      !! The mixture density at cell faces
      REAL(KIND=dp),DIMENSION(:,:,:),allocatable,INTENT(INOUT)::flux
      !! The convective flux 
      REAL(KIND=dp),INTENT(IN):: vb
    ...
    SUBROUTINE DiffusiveFlux(PGrid,UGrid,VGrid,PCell,UCell,VCell,flux,Eflux,   &
      ...
      !! The subroutine computes the diffusive fluxes and extra correction for diffusive fluxes 
      IMPLICIT NONE
      INTEGER(kind=it4b),INTENT(IN):: idir,jdir
      !! The parameter indicating the subroutine applied for x or y direction 	
      REAL(KIND=dp),INTENT(IN):: vb
    ...
    subroutine Mixing_Procedure(TGrid,TCell,idir,jdir,flux_mix)
      !! The subroutine compute the total flux and redistribute it such that the 
      !! resulted flux doesn't cause any problems to small cell.  
      TYPE(Grid),INTENT(IN):: TGrid
      !! The grid	
      TYPE(Cell),INTENT(IN):: TCell
      !! The cell 
      ...
      do i=ibeg,Isize-idir
        do j=jbeg,Jsize-jdir
          ! The condition specifies which cell is small cell and needed 
          ! to redistribute the flux to neighour cells 
          if(TCell%VofS(i,j)>=0.5d0+epsi.and.TCell%Posnu(i,j)/=-1) then
            ! Determine the first target cell
            ...
            ! Determine the second target cell
            ...
            ! Determine the third target cell
            ...
            do ii=1,3
              ...
              ! Compute the flux difference between target cells and the small cell  
              Fmix(ii)=Fmix(ii)*Beta(ii)/(Beta(ii)*(1.d0-TCell%VofS(i,j))+     &
                                                                   Vtgt(ii))
            end do
            ! Update the flux in the small cell 
            flux_mix(i,j)=flux(i,j)+1.d0/(1.d0-TCell%VofS(i,j))*(sum(Fmix))
            ii=i+INT(sign(1.d0,TCell%nxS(i,j)));jj=j
            ! Compute the new flux for target cells
            if(ii<=Isize-idir.and.ii>=1) then    
